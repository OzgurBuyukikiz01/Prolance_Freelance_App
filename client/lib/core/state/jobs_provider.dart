...
class JobsProvider extends ChangeNotifier {
  ...
  List<JobModel> activeJobsForUser(String userId, {String? fallbackUserName}) =>
      _jobs
          .where(
            (j) =>
                ((j.clientId != null && j.clientId == userId) ||
                    (j.clientId == null &&
                        fallbackUserName != null &&
                        j.clientName == fallbackUserName)) &&
                (j.status == 'open' ||
  ...
  bool isOwnedByCurrentUser(
    ...
    String? fallbackUserName,
      ...
      (job.clientId == null &&
          fallbackUserName != null &&
          job.clientName == fallbackUserName);
  ...
  void _debouncedReloadJobsFromRemote() {
    ...
    _jobsRealtimeDebounce = Timer(const Duration(milliseconds: 400), () async {
  ...
  Future<void> addJob(
    ...
    final toInsert = job.isUserPosted ? job.copyWith(status: 'open') : job;
    ...
    if (SupabaseConfig.isEnabled) {
      ...
      if (inserted != null) {
        ...
        if (merged.isUserPosted && merged.status == 'open') {
          _notifyUserPostLive(merged.title, notifications: notifications, t: t);
        }
      }
    } else {
      _jobs.insert(0, merged);
  ...
  Future<JobModel?> updateJob(
    ...
    if (SupabaseConfig.isEnabled) {
      try {
        final updated = await SupabaseJobRepository.updateAndMap(job);
        if (updated != null) {
          _jobs[index] = updated;
          await _persistJobs();
          notifyListeners();
