...
class NotificationRepository extends ChangeNotifier {
  ...
  /// In-memory notification (e.g. moderation UX when Supabase is off).
  void pushLocal(FeedNotificationItem item) {
    _items.insert(0, item);
    if (!_newItemsController.isClosed) {
      _newItemsController.add(item);
    }
  ...
  /// Persists a notification for the signed-in user, or [pushLocal] on failure.
  Future<void> notifyCurrentUser({
    ...
    try {
      ...
      debugPrint('[NotificationRepository] notifyCurrentUser: $e');
      pushLocal(
        FeedNotificationItem(
          id: 'local_${DateTime.now().millisecondsSinceEpoch}',
          title: title,
